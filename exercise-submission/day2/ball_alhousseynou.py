...
def report_basic_data_properties(filescsv):
    df = pd.read_csv(filescsv)
    nb_rows = len(df.index)
    names_cols = list(df.columns)
    valeurs = (nb_rows, names_cols)
    return valeurs
...
def get_name_of_town_with_highest_elevation(csv_file=None, elev_col="elev_metres"):
    ...
    maxi = data[elev_col][0]
    for i in range(len(data[elev_col])):
        if maxi < data[elev_col][i]:
            maxi = data[elev_col][i]
            ligne = i
...
def get_weather(api_key=None, city_id=None):
    ...
    """
    # add your API key
    url = "http://api.openweathermap.org/data/2.5/forecast?id={}&APPID={}".format(api_key, city_id)
...
def compile_weather_forecast(city_name=None, output_csv_file=None):
    ...
    API_KEY = "4a3045fe842597d9c22a96149c1125a9"
    #
    # # JSON file with city details
    #jfile = open(../data/city.list.json)
